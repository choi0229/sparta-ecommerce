...
export const options = {
    ...
    thresholds: {
        ...
        'http_req_duration{scenario:keyword_search}': ['p(95)<1000'],
        'http_req_duration{scenario:brand_search}':   ['p(95)<1000'],
...
export function keywordSearch() {
    ...
    const res = http.get(
        `${BASE_URL}/products/es/search?keyword=${encodeURIComponent(keyword)}&page=${page}&size=${size}`,
        { tags: { name: 'keyword_search' }, timeout: '10s' }
    );
...
export function brandSearch() {
    ...
    const res = http.get(
        `${BASE_URL}/products/es/search?brandName=${encodeURIComponent(brand)}&page=${page}&size=${size}`,
        { tags: { name: 'brand_search' }, timeout: '10s' }
    );
...
export function complexSearch() {
    ...
    const url = `${BASE_URL}/products/es/search?keyword=${encodeURIComponent(keyword)}&brandName=${encodeURIComponent(brand)}&categoryId=${category}&page=${page}&size=${size}`;

    const res = http.get(url, { tags: { name: 'complex_search' }, timeout: '10s' });
...
export function productDetail() {
    ...
    const res = http.get(
        ...
        { tags: { name: 'product_detail' }, timeout: '10s' }
    );
