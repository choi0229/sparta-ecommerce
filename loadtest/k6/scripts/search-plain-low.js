...
// LIKE 검색 저부하 (인덱스 없음 기준선) → /api/products/plain/search
...
export const options = {
    ...
    thresholds: {
        ...
        'http_req_duration{scenario:keyword_search}': ['p(95)<10000'],
        'http_req_duration{scenario:brand_search}':   ['p(95)<10000'],
        'http_req_duration{scenario:complex_search}': ['p(95)<10000'],
...
export function keywordSearch() {
    ...
    const res = http.get(`${BASE_URL}/products/plain/search?keyword=${encodeURIComponent(keyword)}&page=${randomIntBetween(0, 10)}&size=${PAGE_SIZES[randomIntBetween(0, 2)]}`, { tags: { name: 'keyword_search' }, timeout: '10s' });
...
export function brandSearch() {
    ...
    const res = http.get(`${BASE_URL}/products/plain/search?brandName=${encodeURIComponent(brand)}&page=${randomIntBetween(0, 10)}&size=${PAGE_SIZES[randomIntBetween(0, 2)]}`, { tags: { name: 'brand_search' }, timeout: '10s' });
...
export function complexSearch() {
    ...
    const res = http.get(`${BASE_URL}/products/plain/search?keyword=${encodeURIComponent(keyword)}&brandName=${encodeURIComponent(brand)}&categoryId=${category}&page=${randomIntBetween(0, 5)}&size=${PAGE_SIZES[randomIntBetween(0, 2)]}`, { tags: { name: 'complex_search' }, timeout: '10s' });
