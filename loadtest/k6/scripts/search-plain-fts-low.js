...
// FTS 검색 저부하 (product_plain) → /api/products/plain/fts/search
...
export function keywordSearch() {
    ...
    const res = http.get(`${BASE_URL}/products/plain/fts/search?keyword=${encodeURIComponent(keyword)}&page=${randomIntBetween(0, 10)}&size=${PAGE_SIZES[randomIntBetween(0, 2)]}`, { tags: { name: 'keyword_search' }, timeout: '10s' });
...
export function brandSearch() {
    ...
    const res = http.get(`${BASE_URL}/products/plain/fts/search?brandName=${encodeURIComponent(brand)}&page=${randomIntBetween(0, 10)}&size=${PAGE_SIZES[randomIntBetween(0, 2)]}`, { tags: { name: 'brand_search' }, timeout: '10s' });
...
export function complexSearch() {
    ...
    const res = http.get(`${BASE_URL}/products/plain/fts/search?keyword=${encodeURIComponent(keyword)}&brandName=${encodeURIComponent(brand)}&categoryId=${category}&page=${randomIntBetween(0, 5)}&size=${PAGE_SIZES[randomIntBetween(0, 2)]}`, { tags: { name: 'complex_search' }, timeout: '10s' });
